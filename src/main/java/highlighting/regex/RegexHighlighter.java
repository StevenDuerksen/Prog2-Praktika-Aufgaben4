...
public class RegexHighlighter extends SyntaxHighlighter {
  ...
  public List<HighlightRegion> collectMatches(String text) {
    ...
    for (Token token : MiniJavaTokens.defaultTokens()) {
      ...
      while (matcher.find()) {
        regions.add(new HighlightRegion(matcher.start(), matcher.end(), colourFor(token)));
      }
    }
  ...
  // Kleine Hilfsmethode um funny stuff zu machen. Das verwenden von Color als identifier ist
  // unsauber aber hatte keine Lust nen Token-Typ oder so hinzuzufügen lol
